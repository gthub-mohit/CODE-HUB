...
async function handleAcceptedSolution(payload) {
  ...
  // Get platform-specific repo name
  const platformFolder = platform || 'CodeChef';
  let repoName;

  if (platformFolder === 'CodeChef') {
    repoName = config.repoCodeChef;
    if (!repoName) {
      throw new Error('CodeChef repository not configured. Please open CodeHub popup to set up.');
    }
  } else if (platformFolder === 'LeetCode') {
    repoName = config.repoLeetCode;
    if (!repoName) {
      throw new Error('LeetCode repository not configured. Please open CodeHub popup to set up.');
    }
  } else if (platformFolder === 'Codeforces') {
    repoName = config.repoCodeforces;
    if (!repoName) {
      throw new Error('Codeforces repository not configured. Please open CodeHub popup to set up.');
    }
  } else {
    // Fallback for unknown platforms
    repoName = config.repoCodeChef || config.repoLeetCode;
    if (!repoName) {
      throw new Error('No repository configured. Please open CodeHub popup to set up.');
    }
  }

  // Build folder structure: YYYY-MM/PROBLEM.ext (no platform prefix since each repo is platform-specific)
  const date = new Date(timestamp);
  const dateFolder = formatDateFolder(date); // e.g., "2026-08"
  const extension = getFileExtension(language);
  const fileName = `${problemCode}${extension}`;
  const filePath = `${dateFolder}/${fileName}`;

  console.log('[CodeHub] Processing:', platform, '/', problemCode, '→', repoName, '/', filePath);

  // Build commit message
  const commitMessage = `Solved ${problemCode} on ${platformFolder} [${language}]`;
  ...
  chrome.notifications.create({
    ...
    title: `Pushed to GitHub! [${platformFolder}]`,
    message: `${problemCode} → ${repoName}/${filePath}`,
    priority: 2
  });

  console.log('[CodeHub] Success:', repoName, '/', filePath);
...
async function pushSolutionFile(options) {
  const { token, username, repo, filePath, content, commitMessage, 
          problemCode, language, sourceUrl, date, platform } = options;
...
async function updateDateReadme(options) {
  const { token, username, repo, dateFolder, problemCode, language, sourceUrl, fileName, platform } = options;
...
function createNewReadme(dateFolder, problem) {
  const { problemCode, language, sourceUrl, fileName, platform } = problem;
  
  // Parse date from YYYY-MM format
  const [year, month] = dateFolder.split('-');
  ...
  return `# ${platform} Solutions - ${formattedDate}
...
function buildSolutionHeader(code, problemCode, language, sourceUrl, date, platform) {
  ...
  const platformName = (platform || 'CodeChef').padEnd(60, ' ');
...
async function getConfig() {
  return new Promise((resolve) => {
    chrome.storage.local.get(
      ['githubToken', 'githubUsername', 'repoCodeChef', 'repoLeetCode', 'repoCodeforces'],
      resolve
    );
  });
}
